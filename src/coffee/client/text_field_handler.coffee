    ...
    compile: ($element, attr) ->
        ...
        return ($scope, element, iAttrs, controller, transcludeFn) ->
            ...
            onTextChange = () ->
                ...
                $scope.$apply () ->
                    onEditFn $scope, {$op: op}

            element.on "keydown", updateSelection
            element.on "keypress", updateSelection
            element.on "keyup", updateSelection
            element.on "mouseup", updateSelection

            element.on "input", onTextChange

            childrenElems = []

            $scope.$watch editableVal, (t) ->
                $(element).children().remove()
                i = 0
                childrenElems = []
                for text_row in (t.split "\n")[0 ... -1]
                    do (text_row, i) ->
                        childScope = $scope.$new()
                        childScope.$index = i
                        childScope.$line = text_row
                        transcludeFn childScope, (childElem) ->
                            element.append childElem
                            childrenElems.push {elem: $(childElem).find('*').addBack().filter('.contenteditable_single_row').get(0)}
                    i += 1

            doEditFn.assign $scope, (op) ->
                modelText = editableValParsed $scope
                ...
                getContOffset = (totalIndex) ->
                    ...
                    startOfLine = 1 + modelTextNew.lastIndexOf '\n', totalIndex-1
                    offset = totalIndex - startOfLine
                    containerIndex = countNewlines (modelTextNew.substr 0, startOfLine)
                    container = childrenElems[containerIndex].elem
